#include "OLED1315.h"
/* ═══════════════════════════════════════════════════════════════
 *  OLED1315 Driver - Optimized by Claude
 *  Features:
 *    - Software I2C implementation
 *    - GRAM buffer for batch refresh
 *    - Optional refresh parameter (reduce I2C traffic)
 *    - 128x64 monochrome display support
 * ═══════════════════════════════════════════════════════════════ */
#include "oledfont.h"

// OLED地址 (7位地址0x3C，写地址为0x78，读地址为0x79)
...
{
	IIC_Start();
	OLED_SendByte(OLED_I2C_ADDR);  // 发送设备写地址 0x78
	IIC_WaitAck();
	if(mode){OLED_SendByte(0x40);}  // 数据模式
	else{OLED_SendByte(0x00);}  // 命令模式
	IIC_WaitAck();
	OLED_SendByte(data);
	IIC_WaitAck();
	IIC_Stop();
...
void OLED_Refresh(void)
{
	...
	for(i=0;i<8;i++)
	{
		...
		IIC_Start();
		OLED_SendByte(OLED_I2C_ADDR);  // 发送设备写地址
		IIC_WaitAck();
		OLED_SendByte(0x40);  // 数据模式
		IIC_WaitAck();
		for(n=0;n<128;n++)
		{
			OLED_SendByte(OLED_GRAM[n][i]);
			IIC_WaitAck();
		}
		IIC_Stop();
  }
...
void OLED_ShowString(uint8_t x,uint8_t y,uint8_t *chr,uint8_t size1,uint8_t mode,uint8_t refresh)
{
	...
	{
		OLED_ShowChar(x,y,*chr,size1,mode,0);  // 字符串内部不立即刷新
		if(size1==8)x+=6;
		else x+=size1/2;
		chr++;
  }
...
void OLED_Init(void)
{
    HAL_Delay(200);
	  ...
	  // 初始化软件I2C引脚 (GPIO开漏输出模式)
	GPIO_InitTypeDef GPIO_InitStruct = {0};
	GPIO_InitStruct.Pin = GPIO_PIN_6 | GPIO_PIN_7;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_OD;  // 开漏输出，I2C必须
